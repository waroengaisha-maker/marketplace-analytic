...
const props = defineProps<{ rows: { data: Row[] } }>()
const orderFilter = ref('')
const productFilter = ref('')
const sortKey = ref('item_index')
const sortDirection = ref<'asc' | 'desc'>('asc')
const columns = [
    { key: 'item_index', label: 'No. Urut' }, { key: 'order_number', label: 'No. Pesanan' },
...
const filteredRows = computed(() => [...(props.rows.data || [])].filter((row) => String(row.order_number || '').toLowerCase().includes(orderFilter.value.toLowerCase()) && String(row.order_product_name || '').toLowerCase().includes(productFilter.value.toLowerCase())).sort((a, b) => { const result = String(a[sortKey.value] ?? '').localeCompare(String(b[sortKey.value] ?? ''), 'id', { numeric: true, sensitivity: 'base' }); return sortDirection.value === 'asc' ? result : -result }))
...
<template>
    ...
    <div><div class="mb-6"><h1 class="text-2xl font-bold">Reconciliation</h1><p class="mt-2 text-sm text-slate-500">Detail Order dan Income berdasarkan user, nomor pesanan, nama produk, dan item index.</p></div>
        ...
        <div class="overflow-x-auto rounded-xl border border-slate-200 bg-white"><table class="min-w-[1900px] text-left text-sm"><thead class="bg-slate-50 text-xs uppercase text-slate-500"><tr><th v-for="column in columns" :key="column.key" class="cursor-pointer p-3" @click="sortBy(column.key)">{{ column.label }}</th></tr></thead><tbody><tr v-for="(row, index) in filteredRows" :key="row.order_number + '-' + row.item_index" class="border-t"><td class="p-3">{{ index + 1 }}</td><td class="whitespace-nowrap p-3">{{ row.order_number }}</td><td class="min-w-80 p-3">{{ row.order_product_name }}</td><td class="p-3">{{ row.net_quantity }}</td><td class="whitespace-nowrap p-3">{{ money(row.discounted_price) }}</td><td class="p-3">{{ row.quantity ?? 0 }}</td><td class="p-3">{{ row.returned_quantity ?? 0 }}</td><td class="whitespace-nowrap p-3">{{ money(row.order_subtotal) }}</td><td class="whitespace-nowrap p-3">{{ money(row.platform_fee) }}</td><td class="p-3">{{ row.admin_fee_percent.toFixed(2) }}%</td><td class="whitespace-nowrap p-3">{{ money(row.free_shipping_xtra_fee) }}</td><td class="p-3">{{ row.free_shipping_xtra_fee_percent.toFixed(2) }}%</td><td class="whitespace-nowrap p-3">{{ money(row.promo_xtra_service_fee) }}</td><td class="p-3">{{ row.promo_xtra_fee_percent.toFixed(2) }}%</td><td class="whitespace-nowrap p-3">{{ money(row.fee_subtotal) }}</td><td class="whitespace-nowrap p-3">{{ money(row.fee_per_unit) }}</td><td class="whitespace-nowrap p-3">{{ money(row.order_processing_fee) }}</td><td class="whitespace-nowrap p-3 font-semibold">{{ money(row.total_fee) }}</td><td class="whitespace-nowrap p-3">{{ money(row.tax) }}</td></tr><tr v-if="!filteredRows.length"><td colspan="19" class="p-10 text-center text-slate-500">Belum ada data rekonsiliasi.</td></tr></tbody></table></div></div>
</template>